...
*/
class ZZfdsn2Controller extends Zend_Controller_Action
{
	function init()
	{
		$this->initView();
	...
	{
		// Title Browser
		$this->view->title = "Sinkronisasi Data Tim Teaching - Feeder";
		// layout
		$this->_helper->layout()->setLayout('main');
		// control navigasi back
		$this->view->disabled_back="disabled='disabled'";
		// destroy session param
		Zend_Session::namespaceUnset('param_fdsn2');
		// get data prodi
		$prodi = new Prodi();
	...
	function listAction(){
		...
		// navigation
		$this->_helper->navbar('zzfdsn2',0,0,0,0);
		// show data
		...
		if (!$resProdi['data']){
			$this->view->eksis="f";
			$this->view->title="Sinkronisasi Data Tim Teaching";
		}else{
			// feeder side
			...
			// Title Browser
			$this->view->title = "Sinkronisasi Data Tim Teaching Periode ".$per." Program Studi ".$nm_prodi;
			$this->view->prd=$kd_prodi;
			// get data ajar dosen from SIA
			$feederDsn = new FeederDsn();
			$this->view->listDsnSIA = $feederDsn->getKelasDosenTimTeachingByPeriodeProdi($per, $prd);
			// cacah periode
			...
			if($dtKlsFeeder['data']){
				...
				foreach ($dtKlsFeeder['data'] as $dtKls){
					$id_kls[$i]="id_kelas_kuliah='".$dtKls['id_kelas_kuliah']."'";
					$i++;
				}
			}
			// ajar dosen Feeder
			$id_kls=implode(" OR ",$id_kls);
			$getKlsDsn = $feederDsn->GetDosenKelas($token, $id_kls, "", 1000, 0, $url);
