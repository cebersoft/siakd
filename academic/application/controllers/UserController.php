...
/*
	...
	Module		: User Controller -> Controller untuk modul halaman user
*/
class UserController extends Zend_Controller_Action
{
	...
	function indexAction()
	{
		...
		if ($getMenu=="F"){
			...
			// Title Browser
			$this->view->title = "Daftar User Akademik";
			// navigation
			$this->_helper->navbar(0,0,'user/new',0,0);
			$user_acc = new User();
			$this->view->listUserAcc = $user_acc->fetchAll();
		}
	}
	...
	function passwordAction() {
		...
		if ($getMenu=="F"){
			...
			$username = $this->_request->get('id');
			// Title Browser
			$this->view->title = "Password User Akademik";
			// navigation
			$this->_helper->navbar('user',0,0,0,0);
			$user_acc=new User();
			$getUser=$user_acc->getUserAcByUname($username);
