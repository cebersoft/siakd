...
/*
	...
	Module		: SK Bim TA Controller -> Controller untuk modul halaman sk
*/
class SkbimController extends Zend_Controller_Action
{
	function init()
	{
		$this->initView();
	...
	function indexAction(){
		...
		if($getPeriode){
			// Title Browser
			$this->view->title = "SK Pembimbing Periode ".$kd_periode;
			$kd_dsn=$this->kd_dsn;
			// get periode
			...
			// get data sk
			$dosbim=new Dosbim();
			$this->view->listDosbim=$dosbim->getDosbimByPeriodeDosen($kd_periode,$kd_dsn);
		}else{
			$this->view->eksis="f";
			// Title Browser
			$this->view->title = "SK Pembimbing Periode";
		}
	}
