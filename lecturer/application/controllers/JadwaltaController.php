...
class JadwaltaController extends Zend_Controller_Action
{
	...
	function indexAction(){
		...
		if($getPeriode){
			// Title Browser
			$this->view->title = "Jadwal Menguji Periode ".$kd_periode;
			$kd_dsn=$this->kd_dsn;
			// get periode
			...
			// get data
			$kuliah=new KuliahTA();
			$this->view->listKuliah=$kuliah->getKuliahTAByDosjiPeriode($kd_dsn,$kd_periode);
		}else{
			$this->view->eksis="f";
			// Title Browser
			$this->view->title = "Jadwal Menguji";
		}
	}
